...
//MessageType.Warning);
			...
			EditorGUILayout.HelpBox("Please check if your model is used to bind left hand poses",
						MessageType.None);
			...
			MessageType.None);
			handMeshRenderer.showConfidenceAsAlpha = EditorGUILayout.Toggle("ShowConfidenceAsAlpha", handMeshRenderer.showConfidenceAsAlpha);

			EditorGUILayout.LabelField("Source", EditorStyles.boldLabel);

			if (!handMeshRenderer.useRuntimeModel) // use customized hand model
			{
				...
				if (handMeshRenderer.alreadyDetect)
				{
					...
					{
						handMeshRenderer.BonePoses[i] = EditorGUILayout.ObjectField("  " + handMeshRenderer.boneMap[i].DisplayName, handMeshRenderer.BonePoses[i], typeof(Transform), true) as Transform;
					}
			...
			{
				EditorGUILayout.HelpBox("All of hand model and bones are loaded from runtime and model in child will be disabled",
							MessageType.Info);

				handMeshRenderer.alreadyDetect = false;
			}
